...
class MazeGenerator:
    ...
    def generate_maze(self):
        ...
        # Add extra starting point and end point
        self.grid[0][0] = 0  # Extra starting point
        self.grid[self.height - 1][self.width - 1] = 0  # Extra end point
...
class MazeApp:
    ...
    def generate_and_solve_maze(self):
        # Clear previous maze and solutions
        self.canvas.delete("all")
        for solution_id in self.solution_ids:
            self.canvas.delete(solution_id)
    ...
    def draw_maze(self):
        for i in range(MAZE_HEIGHT):
            for j in range(MAZE_WIDTH):
                ...
                elif (i, j) == (0, 1) or (i, j) == (0, 0):  # Starting points
                    self.canvas.create_rectangle(j*CELL_SIZE, i*CELL_SIZE, (j+1)*CELL_SIZE, (i+1)*CELL_SIZE, fill="orange")
                elif (i, j) == (MAZE_HEIGHT - 1, MAZE_WIDTH - 2) or (i, j) == (MAZE_HEIGHT - 1, MAZE_WIDTH - 1):  # Ending points
                    self.canvas.create_rectangle(j*CELL_SIZE, i*CELL_SIZE, (j+1)*CELL_SIZE, (i+1)*CELL_SIZE, fill="orange")
    ...
    def draw_solution(self, solution):
        ...
        for i in range(len(solution) - 1):
            ...
            solution_id.append(self.canvas.create_line(current[1]*CELL_SIZE + CELL_SIZE//2, current[0]*CELL_SIZE + CELL_SIZE//2,
